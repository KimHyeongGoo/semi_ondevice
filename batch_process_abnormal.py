...
from abnormal_monitor import Monitor, PARAM_TABLE_MAP, PREDICT_STEP
...
def run_detection(monitor: Monitor, df: pd.DataFrame, pred_buffer: dict[str, dict]) -> None:
    if df.empty:
        return
    for param in PARAM_TABLE_MAP.keys():
        for _, row in df.iterrows():
            ...
            try:
                ...
                actual_val = None
            monitor.handle_sample(param, ts_sec, actual_val, pred_val, row.get("ProcessRecipeStepID"), row.get("ProcessRecipeStepName"))
        monitor.finalize_event(param)


def process_file(path: str, pool: SimpleConnectionPool, predictor: BatchPredictor, monitor: Monitor) -> None:
    log(f"Processing {path} [load]")
    raw_df = read_csv(path)
    log(f"{path} loaded: {len(raw_df)} rows")
    if raw_df.empty:
        log(f"{path} has no rows after parsing, skipping.")
        return
    dense_df = densify(raw_df)
    log(f"{path} densified: {len(dense_df)} rows")
    log(f"{path} inserting raw -> DB")
    insert_raw_rows(pool, dense_df)
    log(f"{path} raw insert done")
    log(f"{path} predicting")
    pred_rows, pred_buffer = predictor.predict(dense_df)
    for idx, _ in enumerate(TASKS):
        table_name = f"pred_proc{idx}"
        rows = pred_rows.get(table_name, [])
        log(f"{path} saving predictions -> {table_name} ({len(rows)} rows)")
        insert_pred_rows(pool, predictor.pred_col_names[idx], rows, table_name)
    log(f"{path} anomaly detection")
    run_detection(monitor, dense_df, pred_buffer)
    log(f"Completed {path}")


def main():
    base_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "abnormal_data")
    files = find_csv_files(base_dir)
    if not files:
        log("No CSV files found under abnoraml_data.")
        return
    pool = SimpleConnectionPool(1, 5, **DB_CONF)
    predictor = BatchPredictor(pool)
    monitor = Monitor()
    for idx, csv_path in enumerate(files, start=1):
        log(f"[{idx}/{len(files)}] start {csv_path}")
        process_file(csv_path, pool, predictor, monitor)
        log(f"[{idx}/{len(files)}] done {csv_path}")
    pool.closeall()
