...
def process_file(path: str, pool: SimpleConnectionPool, predictor: BatchPredictor, detector: BatchAnomalyDetector) -> None:
...
def main():
    ...
    if not files:
        log("No CSV files found under abnormal_data.")
        log(base_dir)
        return
    ...
    for idx, csv_path in enumerate(files, start=1):
        ...
        process_file(csv_path, pool, predictor, detector)
