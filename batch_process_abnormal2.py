...
def fetch_data_from_table(pool: SimpleConnectionPool, table_name: str, start_ts: datetime = None, end_ts: datetime = None) -> pd.DataFrame:
    """DB에서 특정 테이블의 데이터를 가져옵니다.
    
    Args:
        pool: 데이터베이스 연결 풀
        table_name: 테이블 이름 (예: 'rawdata20251102')
        start_ts: 시작 시간 (옵션, 해당 날짜의 시작 시간 필터링)
        end_ts: 종료 시간 (옵션, 해당 날짜의 종료 시간 필터링)
    
    Returns:
        데이터프레임
    """
    add_columns = []
    for predict_column in sum(TASKS, []):
        if 'Temp_Act_' in predict_column:
            temp_pos = predict_column.split('_')[-1]
            for add_col in TEMP_ADD_COLUMNS:
                add_cols_name = add_col + temp_pos
                if add_cols_name not in add_columns:
                    add_columns.append(add_cols_name)
    
    colnames = ', '.join([f'"{col}"' for col in ["Timestamp"] + SELECTED_COLS + add_columns + ["ProcessRecipeStepID", "ProcessRecipeStepName"]])
    ...
    try:
        ...
        log(f"Failed to load from {table_name}: {e}")
        return pd.DataFrame()
    finally:
        pool.putconn(conn)
...
class BatchAnomalyDetector:
    ...
    def get_overlapping_violation_type(self, param: str, start_time: datetime, end_time: datetime):
        ...
        try:
            ...
            log(f"[violation_type 조회 오류] param={param}: {e}")
        finally:
            self.pool.putconn(conn)
        return None
        ...
        try:
            with conn.cursor() as cur:
                message_payload = {
                    ...
                    "violation_type": self._to_py(violation_type),
                }
...
def process_table_data(table_name: str, df: pd.DataFrame, predictor: BatchPredictor, detector: BatchAnomalyDetector) -> None:
    """테이블 데이터를 처리합니다 (예측 -> 이상감지 -> 이상데이터 저장)"""
    log(f"[{table_name}] Processing data: {len(df)} rows")
    if df.empty:
        log(f"[{table_name}] Data has no rows, skipping.")
        return
    
    # 1. 실제값 불러옴 (이미 완료)
    log(f"[{table_name}] Data loaded: {len(df)} rows")
    
    # 2. 데이터 densify
    dense_df = densify(df)
    log(f"[{table_name}] Data densified: {len(dense_df)} rows")
    
    # 3. 예측
    log(f"[{table_name}] Predicting")
    pred_rows, pred_buffer = predictor.predict(dense_df)
    
    # 4. 이상감지 및 이상데이터 저장
    log(f"[{table_name}] Anomaly detection")
    detector.run_detection(dense_df, pred_buffer)
    
    log(f"[{table_name}] Completed processing")


def main():
    # 2025-11-02 ~ 2025-12-25 사이의 데이터를 DB에서 가져옴
    start_date = "2025-11-02"
    end_date = "2025-12-25"
    
    log(f"Starting batch processing from {start_date} to {end_date}")
    ...
    # 각 테이블별로 처리: 실제값 불러옴 -> 예측 -> 이상감지 -> 이상데이터 저장
    while current_date <= end_dt:
        table_idx += 1
        table_name = f'rawdata{current_date.strftime("%Y%m%d")}'
        
        log(f"[{table_idx}/{total_tables}] Processing {table_name}")
        
        # 시작 날짜와 종료 날짜인 경우 시간 필터 적용
        if current_date.date() == start_dt.date():
            start_ts = start_dt
        else:
            start_ts = current_date.replace(hour=0, minute=0, second=0)
        
        if current_date.date() == end_dt.date():
            end_ts = end_dt
        else:
            end_ts = current_date.replace(hour=23, minute=59, second=59)
        
        # 1. 실제값 불러옴
        df = fetch_data_from_table(pool, table_name, start_ts, end_ts)
        
        if not df.empty:
            # 2. 예측 -> 3. 이상감지 -> 4. 이상데이터 저장
            process_table_data(table_name, df, predictor, detector)
            processed_tables += 1
        else:
            log(f"[{table_idx}/{total_tables}] {table_name} has no data, skipping.")
        
        current_date += timedelta(days=1)
    
    pool.closeall()
    log(f"Batch processing finished. Processed {processed_tables}/{total_tables} tables.")
